...
<?php include "config/server.php";
	...
	$sqlgabung = mysqli_query($sqlconn,"SELECT * FROM  `cbt_siswa` s LEFT JOIN cbt_ujian u ON (s.XKodeKelas = u.XKodeKelas or u.XKodeKelas = 'ALL') 
	...
	$sqluser = mysqli_query($sqlconn,"SELECT u.*,m.XNamaMapel FROM cbt_ujian u LEFT JOIN cbt_paketsoal p on p.XKodeSoal = u.XKodeSoal and p.XKodeMapel = u.XKodeMapel left join cbt_mapel m on u.XKodeMapel = m.XKodeMapel 
		WHERE u.XKodeSoal='$xkodesoal' and u.XStatusUjian = '1'");
	$so = mysqli_fetch_array($sqluser);
	$sopil = $so['XJumPilihan'];
	$listen =$so['XListening'];

	$sql = mysqli_query($sqlconn,"SELECT j.Urut as Urut, j.XJawaban as XJawaban, j.XJawabanEsai as XJawabanEsai , c.XNomerSoal as XNomerSoal, j.XRagu as XRagu, c.XJenisSoal as XJenisSoal 
...
<?php include "config/server.php";
	...
	$sqluser = mysqli_query($sqlconn,"SELECT u.*,m.XNamaMapel FROM cbt_ujian u LEFT JOIN cbt_paketsoal p on p.XKodeSoal = u.XKodeSoal and p.XKodeMapel = u.XKodeMapel 
	...
	<section class="page-section soal-navigation">
		<div class="container1" style="margin-right:-100px; margin-left:-50px;">
			...
			<div >
					...
					<?php 	$cekragu = mysqli_num_rows(mysqli_query($sqlconn,"select * from cbt_jawaban where XRagu ='1' and XKodeSoal ='$xkodesoal' and XTokenUjian = '$xtokenujian'  and XUserJawab = '$user'")); 
							if($cekragu>0){ ?>
